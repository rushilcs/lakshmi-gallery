...
async function buildAdminGalleryPayload(galleryId: string): Promise<unknown | null> {
  ...
  const watermark_url =
    gallery.watermark_asset_key != null
...
async function handleDeleteGalleryImages(req: Request, res: Response): Promise<void> {
  const galleryId =
    typeof req.params.galleryId === "string" ? req.params.galleryId : "";
...
adminRouter.post("/galleries/:galleryId/sort", async (req, res) => {
  ...
  await setGalleryDefaultSort(
    req.params.galleryId,
    parsed.data.default_sort as DefaultSort,
  );
  res.json({ ok: true });
});
