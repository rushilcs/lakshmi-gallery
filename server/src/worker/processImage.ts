...
export async function processImageJob(job: ProcessImageJob): Promise<void> {
  ...
  if (image.processing_status === "completed" && image.thumb_key && image.preview_key) return;
  ...
  let watermarked_thumb_key: string | null = null;
  let watermarked_preview_key: string | null = null;
