...
import type { FaceProvider } from "../faces/FaceProvider.js";
import type { IndexFacesJob } from "../jobs/types.js";

export async function runIndexFacesJob(
  job: IndexFacesJob,
  faceProvider: FaceProvider,
): Promise<void> {
  const imagesWithBytes: Array<{ imageId: string; bytes: Buffer }> = [];
  for (const imageId of job.imageIds) {
    const image = await getImageById(imageId);
    if (!image) continue;
    const bytes = await readBufferFromStorage(image.original_key);
    if (bytes) imagesWithBytes.push({ imageId, bytes });
  }
  if (imagesWithBytes.length === 0) return;
