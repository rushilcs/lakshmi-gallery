...
export async function getGalleryByShareToken(share_token: string): Promise<Gallery | null> {
  const [row] = await getDb().select().from(galleries).where(eq(galleries.shareToken, share_token));
  return row ? rowToGallery(row) : null;
}

export async function setGalleryPublish(galleryId: string, isPublished: boolean): Promise<void> {
  await getDb()
    .update(galleries)
...
export async function setGalleryCover(galleryId: string, coverImageId: string | null): Promise<void> {
  await getDb().update(galleries).set({ coverImageId }).where(eq(galleries.id, galleryId));
}

export async function setGalleryWatermarkEnabled(galleryId: string, enabled: boolean): Promise<void> {
  await getDb().update(galleries).set({ watermarkEnabled: enabled }).where(eq(galleries.id, galleryId));
}

export async function setGalleryWatermarkAsset(galleryId: string, key: string): Promise<void> {
  await getDb().update(galleries).set({ watermarkAssetKey: key }).where(eq(galleries.id, galleryId));
}

export async function setGalleryWatermarkPosition(
  galleryId: string,
  position: {
