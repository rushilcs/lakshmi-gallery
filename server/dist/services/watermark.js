...
export async function createThumbnail(original) {
...
export async function createPreview(original) {
    return sharp(original)
        .rotate()
...
export async function applyWatermarkOverlay(input) {
    ...
    const usePortrait = isPortrait &&
        (input.scale_portrait != null || input.x_pct_portrait != null || input.y_pct_portrait != null);
    const scale = Math.max(0.05, Math.min(0.6, usePortrait ? (input.scale_portrait ?? 0.2) : (input.scale ?? 0.2)));
