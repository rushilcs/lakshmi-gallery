...
function rowToCluster(row) {
    return {
        ...
        representative_image_ids: row.representativeImageIds,
...
export async function linkImageToCluster(image_id, person_cluster_id) {
    await getDb()
...
export async function listPersonClustersByGallery(galleryId) {
...
export async function getImageIdsForCluster(clusterId) {
    const rows = await getDb()
