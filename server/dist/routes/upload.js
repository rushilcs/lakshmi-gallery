...
function deriveExt(fileName, contentType) {
    ...
    if (byMime) {
        ...
        if (byMime === "tiff")
            return "tiff";
        if (IMAGE_EXTENSIONS.has(byMime))
            return byMime;
    }
    const byName = path.extname(fileName).replace(".", "").toLowerCase();
    if (byName && IMAGE_EXTENSIONS.has(byName))
        return byName === "jpeg" ? "jpg" : byName;
    return "jpg";
}
function buildOriginalKey(galleryId, photoId, ext) {
...
uploadRouter.post("/complete", async (req, res) => {
    ...
    try {
        ...
        const invalid = parsed.data.uploaded.find((item) => !isImageFile(item.content_type || "", item.original_filename ?? item.original_key));
        if (invalid) {
