...
export async function ingestUploadBatch(input: {
  ...
  const faceInputs: Array<{ imageId: string; bytes: Buffer }> = [];
  ...
  for (const item of input.uploaded) {
    try {
      ...
      if (exifDate instanceof Date) taken_at = exifDate.getTime();

      let wmThumb: string | null = null;
      let wmPreview: string | null = null;
...
export async function regenerateWatermarkedDerivatives(
  gallery_id: string,
  galleryOverride?: Gallery | null,
): Promise<void> {
  const gallery = galleryOverride ?? (await getGalleryById(gallery_id));
  if (!gallery || !gallery.watermark_asset_key) return;
